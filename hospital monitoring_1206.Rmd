...
mrp_pcr_fit = readRDS("~/Dropbox (University of Michigan)/MRRP_918 Data/fit_pcr_1123_asymptomatic.rds")
...
fit_pcr_symptomatic=readRDS("~/Dropbox (University of Michigan)/MRRP_918 Data/fit_pcr_1123_symptomatic.rds")

fit_pcr_symptomatic=readRDS("~/Dropbox (University of Michigan)/MRRP_918 Data/fit_pcr_1123_symptomatic_strongprior.rds")

fit_pcr_symptomatic=readRDS("~/Dropbox (University of Michigan)/MRRP_918 Data/fit_pcr_1123_symptomatic_maineffect.rds")
...
```{r, echo = FALSE, message=FALSE}
state_data <- read.csv("./covid_report_county_region_date_1123.csv")
...
ggplot(data=combine_v_asym, aes(x=week,y=est)) +
    geom_point(aes(shape=type))+
  geom_line(aes(linetype=type),alpha=0.4)+
  facet_grid(. ~ type)
...
plot_mrpc=ggplot(data=p_mrpc, aes(x=week,y=Comasym))+
  ...
  scale_x_continuous(breaks=seq(from=min(combine_v_asym$week),to=max(combine_v_asym$week) - 1, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+theme(axis.text.x = element_text(hjust = -1, size=8),axis.text.y = element_text(size=8), axis.title.x = element_text(size=8), axis.title.y = element_text(size=8), legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
ggplot()+
  ...
  scale_x_continuous(breaks=seq(from=20,to=max(as.numeric(as.character(sex_prevalence$pcr_week_ind))), by = 4),
                     # labels = c(
                     #                                            "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                     #                                            "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
...
ggplot()+
  ...
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_hosp_sym)-1, by = 4),
   #                   labels = c(
...
ggplot()+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.35))+
   scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_state_sym)-1, by = 4),
...
ggplot()+
  ...
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))
ggsave(width = 5, height = 3, units = "in", device='png',file="plot/ppc_asym.png") 
...
ggplot()+
  ...
  labs(title = "Posterior predictive check for symptomatic patients", x = "", y = "Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))+
...
plot_hosp=ggplot(data=state_hosp, aes(x=pcr_week_ind,y=prevalence))+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.45))+
  scale_x_continuous(breaks=seq(from=min(state_hosp$pcr_week_ind)+1,to=max(state_hosp$pcr_week_ind) - 1, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+theme(axis.text.x = element_text(hjust = -1, size=8),axis.text.y = element_text(size=8), axis.title.x = element_text(size=8), axis.title.y = element_text(size=8), legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
#a
MRP_Positivity <- data.frame(rep(seq(from = 36, to = 45, by = 1), 2),
...
plot_mrp_sn_lg <- ggplot()+
 ...
 # geom_line(data =District_hospitalization_summary, aes(x = week_ind, y = total_hospitalization))+
  scale_x_continuous(breaks=seq(from=36,to=45, by = 1),labels = c("Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9"))+
...
plot_a<- ggplot()+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.4))+
  scale_x_continuous(breaks=seq(from=36,to=45, by = 1),labels = c(
...
#c
plot_c <- ggplot()+
  geom_line(data =MRP_summary, aes(x = week, y = est, color = "MRP"))+
...
county = patient_data$county_stan,
sex_age=patient_data$sex_age,
J_spec=14, y_spec=c(0, 368, 30, 70, 1102, 300, 311, 500, 198, 99, 29, 146, 105, 50), n_spec=c(0, 371, 30, 70, 1102, 300, 311, 500, 200, 99, 31, 150, 108, 52), J_sens=4, y_sens=c(70, 78, 27, 25), n_sens=c(100, 85, 37, 35), logit_spec_prior_scale=0.2, logit_sens_prior_scale=0.2, intercept_prior_mean=0, intercept_prior_scale=2.5, coef_prior_scale=2.5,coef_prior_scale_time=5, J=2*3*5*2, N_pop=poststratification_table_nozip$N, N_acs = county.ps, kn=dim(B)[1],B=B, J_pred=length(unique(patient_data$pcr_week_ind))+4, B_pred=B_pred)
...
fit_pcr_spline <- stan(file = "MRP_pcr_spline.stan", data =patient_pps_data_spline ,refresh=0, cores=7,iter = 5000,chains=2) 
