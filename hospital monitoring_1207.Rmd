...
ggplot(data =PCR_prevalence_by_week_symptomatic, aes(x = as.numeric(pcr_week_ind),y = prevalence))+
  ...
  scale_x_continuous(breaks=seq(from=min(PCR_prevalence_by_week_symptomatic$pcr_week_ind),to=max(PCR_prevalence_by_week_symptomatic$pcr_week_ind) - 2 , by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+
...
ggplot(data =count_comparison, aes(x = count_comparison[,1] ,y =  count_comparison[,2] ))+
  ...
  scale_x_continuous(breaks=seq(from=min(count_comparison$week),to=max(count_comparison$week) - 2, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+
...
ggplot(data=est_raw_symptomatic, aes(x=week,y=est,group=type,colour=type))+
  ...
  labs (title = "", x = "Week", y = "PCR Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.6))+
  scale_x_continuous(breaks=seq(from=min(est_raw_symptomatic$week),to=max(est_raw_symptomatic$week)- 2, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
...
combine_plot = ggplot(data=combine_v, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(combine_v$week),to=max(combine_v$week)- 2, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+theme(axis.text.x = element_text(hjust = -1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
combine_plot_asym = ggplot(data=combine_v_asym, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.6))+
  scale_x_continuous(breaks=seq(from=min(combine_v_asym$week),to=max(combine_v_asym$week) - 2, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+theme(axis.text.x = element_text(hjust = -1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
ggplot()+
  ...
  scale_x_continuous(breaks=seq(from=20,to=max(as.numeric(as.character(sex_prevalence$pcr_week_ind))) - 1, by = 4),
                     # labels = c(
                     #                                            "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                     #                                            "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
...
ggplot()+
  ...
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_hosp_sym)-2, by = 4),
   #                   labels = c(
...
ggplot()+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.35))+
   scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_state_sym)-2, by = 4),
...
plot_hosp=ggplot(data=state_hosp, aes(x=pcr_week_ind,y=prevalence))+
  ...
  scale_x_continuous(breaks=seq(from=min(state_hosp$pcr_week_ind)+1,to=max(state_hosp$pcr_week_ind) - 2, by = 4),labels = c("May","Jun","July","Aug","Sep","Oct","Nov"))+
  theme_bw()+theme(axis.text.x = element_text(hjust = -1, size=8),axis.text.y = element_text(size=8), axis.title.x = element_text(size=8), axis.title.y = element_text(size=8), legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
fit_pcr_spline <- readRDS("fit_pcr_spline_1130_asymptomatic.rds")
...
##########Posterior prediction distribution
fit_pcr_spline_symptomatic  <- readRDS("fit_pcr_spline_1130_symptomatic.rds")
