...
state_data_by_week <- state_data %>% 
  filter(district == '1',
         week_ind != 37) %>% #Remove latest week imcomplete data 
  group_by(week_ind) %>% 
  summarize(
    total_test = sum(covid_test),
    prevalence = sum(covid_count)/total_test
...
ggplot(data =state_data_by_week, aes(x = week_ind,y = prevalence))+
  geom_point() +
  geom_line()+
  labs(title = "", x = "Week", y = "Prevalence")+
  theme_bw()
...
pcr_data_stan_GP <- list(N=dim(patient_data)[1], y=patient_data$y_response,
                   J_time=length(unique(patient_data$pcr_week_ind)),
                   male=patient_data$sex_stan, race=as.numeric(patient_data$race_stan), age=as.numeric(patient_data$age_stan), 
week=patient_data$pcr_week_ind-17,
week_real=c(rep(1:15)),
county = patient_data$county_stan,
sex_age=patient_data$sex_age,
J_spec=14, y_spec=c(0, 368, 30, 70, 1102, 300, 311, 500, 198, 99, 29, 146, 105, 50), n_spec=c(0, 371, 30, 70, 1102, 300, 311, 500, 200, 99, 31, 150, 108, 52), J_sens=4, y_sens=c(70, 78, 27, 25), n_sens=c(100, 85, 37, 35), logit_spec_prior_scale=0.2, logit_sens_prior_scale=0.2, intercept_prior_mean=0, intercept_prior_scale=2.5, coef_prior_scale=2.5, J=2*3*4*2, N_pop=poststratification_table_nozip$N, N_acs = county.ps)

#fit_pcr_GP <- stan(file = "MRP_pcr_GP.stan", data = pcr_data_stan_GP,refresh=0, cores=8,iter = 10000,chains=2) 
#print(fit_pcr_GP, pars=c("p_avg_h","p_avg_c"), digits=3)
#saveRDS(fit_pcr_GP, "fit_pcr_GP.rds")
fit_pcr_GP <- readRDS("./fit_pcr_GP.rds")

###Plotting
est_raw_GP = data.frame(cbind(rep(c("Raw","MRP-hospital","MRP-community"),each=length(unique(PCR_prevalence_by_week$pcr_week_ind))),rep(PCR_prevalence_by_week$pcr_week_ind,3),c(PCR_prevalence_by_week$prevalence,apply(rstan::extract(fit_pcr_GP,"p_avg_h", permuted = TRUE)[[1]],2,mean),apply(rstan::extract(fit_pcr_GP,"p_avg_c", permuted = TRUE)[[1]],2,mean)),c(PCR_prevalence_by_week$prevalence,apply(rstan::extract(fit_pcr_GP,"p_avg_h", permuted = TRUE)[[1]],2,quantile, probs=c(0.025)),apply(rstan::extract(fit_pcr_GP ,"p_avg_c", permuted = TRUE)[[1]],2,quantile, probs=c(0.025))),c(PCR_prevalence_by_week$prevalence,apply(rstan::extract(fit_pcr_GP,"p_avg_h", permuted = TRUE)[[1]],2,quantile, probs=c(0.975)),apply(rstan::extract(fit_pcr_GP ,"p_avg_c", permuted = TRUE)[[1]],2,quantile, probs=c(0.975)))))
names(est_raw_GP )=c("type","week","est","low","up")
est_raw_GP $week = as.numeric(as.character(est_raw_GP$week))
est_raw_GP $est = as.numeric(as.character(est_raw_GP$est))
est_raw_GP $low = as.numeric(as.character(est_raw_GP$low))
est_raw_GP $up = as.numeric(as.character(est_raw_GP$up))

ggplot(data=est_raw_GP, aes(x=week,y=est,group=type,colour=type))+
  geom_point(aes(shape=type))+
  geom_line(aes(linetype=type))+
  labs (title = "", x = "Week", y = "PCR Prevalence")+
  theme_bw()+theme(legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
