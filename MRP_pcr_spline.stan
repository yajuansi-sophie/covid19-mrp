...
model {
  ...
  a_time ~ normal(mu_time, sigma_time);
...
generated quantities {
  ...
  vector[J_pred] p_avg_h_pred;
  vector[J_pred] p_avg_c_pred; 
  vector[J_pred] mu_time_pred;
  vector[J_pred] a_time_pred;
...
for (time in 1:J_time){
  ...
  mu_time_pred = to_vector(a * B_pred);
  for (j in 1:J_pred){
      a_time_pred[j] = normal_rng(mu_time_pred[j], sigma_time);
  }

for (time in 1:J_pred){
  count_pred = 1;
  for (i_male in 0:1) {
    for (i_age in 1:5) {
      for (i_race in 1:3) {
          for (i_county in 1:2){
          p_pop_pred[count_pred] = inv_logit(b[1]
                                   + b[2] * (i_male - 0.5)
                                   + a_race[i_race]
                                   + a_age[i_age]+a_time_pred[time]+i_male*a_sexage[i_age] + a_county[i_county]);
          count_pred += 1;
      }
    }
  }
  }
  p_avg_h_pred[time] = sum(N_pop .* p_pop_pred) / sum(N_pop);
  p_avg_c_pred[time] = sum(N_acs .* p_pop_pred) / sum(N_acs);
}
