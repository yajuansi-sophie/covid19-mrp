...
In the presence of an epidemic, it is important for both governmental and healthcare organizations to have indications of changes in rates of exposure among patients and the general population. We present here an approach to collecting and analyzing data on viral exposure among patients in a hospital system and performing statistical adjustment to estimate viral incidence and trends in the community. We are currently applying this monitoring and analysis protocol in a hospital system in Indiana, and this could serve as a model for other medical groups to perform local data collection and analysis, ultimately with an eye to linking to national data collection efforts and population generalizability.


# The Problem

Knowledge of incidence and trends of viral behavior in communities is important and, as yet, poorly characterized.  In the absence of publicly mandated or academically sponsored random testing, interested parties have been left to extrapolate impressions of community viral behavior from nonrandom sources.  Generally, public health professionals have relied on state-sourced positivity rates and raw numbers of positive tests in any given jurisdiction as proxies for the true viral burden. Unfortunately, these presumed proxies are subject to significant selection bias, as most testing protocols understandably target symptomatic and presumed-exposed populations. Further, the tests have been applied with different criteria over time and geography according to test availability, perceived community viral burden, and disparate clinical or political testing norms. Indeed, academic and public health entities have found it challenging to translate current available prevalence data into a characterization of true virus behavior.  Absent broad randomized testing—which has not occurred and does not seem to be forthcoming—we need a means of obtaining more nearly random data or a method of normalizing currently available data to the true underlying incidence. 

# Testing Regime

There are two main types of tests for COVID-19. The first detects viral RNA using molecular methods such as polymerase chain reaction (PCR). These tests are highly specific because they are based on the unique genetic sequence of SARS-CoV-2. However, the sensitivity of these tests varies based on both the timing and the way the sample is collected. In contrast to nucleic acid testing, which directly detects the virus, antibody or serology testing is used to detect an immune response in the patient. Most patients have detectable IgG antibodies by day 14 following symptom onset, and the likelihood of detection initially increases over time, though there is substantial evidence that these antibodies become less detectable after a few months.

# Proxy Sample

We were fortunate in that the anesthesia professionals in our hospital system were sufficiently concerned about asymptomatic viral shedding to want to preoperatively test all patients. All elective surgical patients are presumptive asymptomatic, as anyone exhibiting symptoms or presenting a recent history of known viral exposure would have surgery cancelled or deferred. This population presented a potentially valuable resource. There is a broad age, racial/ethnic, and economic diversity to this group, and its only overt correlation to disease status is that it is selected for a lack of symptoms and a negative exposure history. Such a group should behave more like the population as a whole with respect to the disease than the symptomatic population. Naturally, we would have preferred a random sample chosen without reference to symptom status, but we anticipate that the current population, if not ideal, is a promising proxy for inferring a true random sample. If we venture the hypothesis that, for any uniform demographic, the ratio of asymptomatic-to-symptomatic viral infection is constant (an unverified but reasonable assumption), then the asymptomatic population in a community would vary in strict ratio with overall prevalence. Thus, if we are able to normalize our test population to the true community demographics, we should have a very close approximation to a true random sample.
Further, if we were to extend testing beyond acute sampling for viral RNA to also include assay for IgG antibody, we would have an even better random proxy for post infectious immune prevalence. That is, this group is not random with respect to acute disease, but should be quite nearly random with respect to a history of disease one month or more ago: the precise group the population IgG status wishes to identify. We were able to institute routine IgG testing for any preoperative patient who was otherwise requiring preoperative blood tests. Admittedly, this group trends somewhat older and sicker than the group at large (undergoing the PCR test), but still seems a fairly representative sample for our community at large; in particular, a truly representative sample if we are able to normalize for these skewed demographics.
Of crucial importance, our sample group varies from a true random sample in predictable ways.  Both the PCR and IgG groups are certainly nonrandom, but both are selected rigorously for asymptomatic/nonexposed status, and their demographics are well documented within the hospital electronic health records (EHR).  It remains only to normalize our sample population to the true demographics of the community to achieve random sampling, to any reasonable approximation.  By poststratification to match demographics of the target community, we expect that our model is able to systematically adjust for these particular selection biases, allowing us to extrapolate our observations to some conclusions about the true population, to a reasonable degree of confidence.

# Measurement

We anticipated some statistical challenges. Sensitivities and specificities vary greatly among available testing regimens. We are using Abbott tests for which these parameters are reasonably stable and well analyzed. The PCR RNA test shares sensitivities similar to all the better tests out there. They all report a 70% sensitivity, but that is standardized by detection of RNA in strongly symptomatic patients. There is a fair amount of data to suggest that asymptomatic and pre-symptomatic patients are both harder to detect than the highly symptomatic ones and that the date of infection and symptom onset have a large effect on the sensitivity (Kucirka et al. 2020). These effects would need to be acknowledged and, to the degree possible, accounted for in the model. Specificity is likely to be theoretically 100%; the only false positives should be cross contaminated or switched samples. A random match to the RNA sequence of interest by a non SARS-CoV-2 virus at the detection threshold should be virtually impossible mathematically with the viruses involved. However, even very small false positive rates can result in large uncertainties about exposure rates if underlying prevalence is low enough. The IgG test also has high sensitivity (70%) and specificity (100%), but we would need these statistics respected in the model.

# Statistical analysis

In addition to adjusting for measurement error, we need to normalize for variation of this population demographics compared to the actual community. We expected that we would be helped by having a fairly representative group to begin with and that poststratification to the target population would help enhance the accuracy of our conclusions.
We are interested in rates of coronavirus exposure, both among hospital patients and in the larger community. We recognize that patients, even asymptomatic patients, are not representative of the general population; nonetheless adjusting for demographics and geography is a start. Multilevel regression and poststratification (MRP) is a standard method of adjustment used in survey research that is particularly effective when sample sizes are small in some demographic or geographic slices of the data (Gelman and Little 1997; Si et al. 2020). We use the Bayesian approach of Gelman and Carpenter (2020) to account for unknown sensitivity and specificity and apply MRP to testing data for population representation, here using the following adjustment variables: sex, age (0-17, 18-34, 35-64, 65-74 and 75+), race (white, black and others), and county (Lake and Porter).

We poststratify to two different populations: patients in the hospital database (i.e., those who have historically and currently obtained care in our regional hospital system) and residents of Lake/Porter County, Indiana. For the hospital, we use the following EHR database to represent the population of inpatients from three hospitals in the Community Health Systems (Community Hospital, St. Catherine Hospital and St. Mary Medical Center). For the community, we use the American Community Survey (ACS) 2014-2018 data from the two counties of interest.
...
where $\pi_i$ is the viral incidence of individual i, $male$ with value 0.5 indicates men and -0.5 for women, $age[i], race[i]$, and $county[i]$ represent age, race and county categories, with a two-way interactiont term $age * male[i]$, $time[i]$ indices the time in weeks when the test result is observed for individual $i$; and the $\alpha$ parameters are vectors of varying intercepts. We assign hierarchical priors to the varying intercepts
\begin{align}
\label{prior}
\alpha^{name} \sim \textrm{normal}(0, \sigma^{name})\mbox{, }\sigma^{name}\sim \textrm{normal}_{+}(0, 2.5)\mbox{, for }name \in {age, race, county, age * male}.
\end{align}
And we set $\alpha^{time} \sim \textrm{normal}(0, \sigma^{time})$, $\sigma^{time}\sim \textrm{normal}_{+}(0, 5)$ to allow a large variation across time.
We perform all computations in Stan and R, and all data and code are available at \url{https://github.com/yajuansi-sophie/covid19-mrp}.


# Pre-study conjectures

We began the data collection with a few hypotheses or speculations. First, that the ratio between asymptomatic and symptomatic patients would be relatively constant, for a uniform demographic distribution. Second, that changes in asymptomatic measurable RNA would precede parallel changes in symptomatic measurable RNA by a couple of days, given the known temporal relationship of viral shedding to the onset of clinical disease. Third, that trends in our asymptomatic RNA positives would therefore parallel and predict the behavior of the virus within the community as a whole.
...
state_data <- read.csv("./covid_report_county_region_date_1121.csv")
...
patient_data <- read_csv(file= "./UM1119.csv", na = c("", "#NUM!"))
...
patient_data_symptomatic <- read_csv(file = "./UM1119sx.csv", na = c("", "#NUM!"))
...
PCR_prevalence_by_week_plot <- ggplot(data =PCR_prevalence_by_week, aes(x = as.numeric(pcr_week_ind),y = prevalence))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
ggsave(PCR_prevalence_by_week_plot, width = 5, height = 3, units = "in", device='png', file="plot/pcr-raw.png")
...
ggsave(IGG_prevalence_by_week_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_raw.png")
...
ggsave(PCR_prevalence_by_week_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr-raw-sym.png")
#Repeat analysis for IGG result
...
ggsave(IGG_prevalence_by_week_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg-raw-sym.png")
...
```{r, echo = FALSE,message=FALSE}
ratio_approximated <- PCR_prevalence_by_week$total_patient[2:30]/ PCR_prevalence_by_week_symptomatic$total_patient[1:29]
count_comparison <- data.frame(PCR_prevalence_by_week$pcr_week_ind[2:30], ratio_approximated)
...
count_comparison_plot <- ggplot(data =count_comparison, aes(x = count_comparison[,1] ,y =  count_comparison[,2] ))+
  geom_point() +
  geom_line()+
  labs(title = "", x = "Week", y = "PCR tests asym/sym")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 30))+
  scale_x_continuous(breaks=seq(from=min(count_comparison$PCR_prevalence_by_week.pcr_week_ind.2.30.),to=max(count_comparison$PCR_prevalence_by_week.pcr_week_ind.2.30.), by = 1),labels = c("May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))
ggsave(count_comparison_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_asym_sym_ct.png")

IGG_ratio_approximated <- IGG_prevalence_by_week$total_patient[1:30]/ IGG_prevalence_by_week_symptomatic$total_patient[1:30]
IGG_count_comparison <- data.frame(IGG_prevalence_by_week$igg_week_ind[1:30], IGG_ratio_approximated)
...
IGG_count_comparison_plot <- ggplot(data =IGG_count_comparison, aes(x = IGG_count_comparison[,1] ,y =  IGG_count_comparison[,2] ))+
  geom_point() +
  geom_line()+
  labs(title = "", x = "Week", y = "Count ratio of asymptomatic and symptomatic IgG tests")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 25))+
  scale_x_continuous(breaks=seq(from=min(IGG_count_comparison$IGG_prevalence_by_week.igg_week_ind.1.30.),to=max(IGG_count_comparison$IGG_prevalence_by_week.igg_week_ind.1.30.), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))

ggsave(IGG_count_comparison_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_asym_sym_ct.png")
...
sex_dist=ggplot()+
  ...
  scale_x_continuous(breaks=seq(from=min(sex_pct$pcr_week_ind),to=max(sex_pct$pcr_week_ind), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26","Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
race_dist=ggplot()+
  ...
  labs (title = "", x = "Week", y = "Proportion")+
  scale_x_continuous(breaks=seq(from=min(race_pct$pcr_week_ind),to=max(race_pct$pcr_week_ind), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26","Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
age_dist=ggplot()+
  ...
  labs (title = "", x = "Week", y = "Proportion")+ 
  scale_x_continuous(breaks=seq(from=min(age_pct$pcr_week_ind),to=max(age_pct$pcr_week_ind), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26","Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
    theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
mrp_pcr_fit = readRDS("fit_pcr_1119_asymptomatic.rds")
...
ggsave(MRP_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_mrp_est.png")
...
MRP_sd_plot <- ggplot(data=est_raw_1, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(est_raw_1$week),to=max(est_raw_1$week), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                ...
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26","Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
ggsave(MRP_sd_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_mrp_sd.png")

#est_raw_1[c(22:23,45:46,68:69),]
...
sym_1119_testing_strongprior <- stan(file = "MRP_pcr_prediction_pps_symptomatic_1119.stan", data = patient_pps_data_symptomatic,refresh=0, cores=7,iter = 10000,chains=2)
saveRDS(sym_1119_testing_strongprior, "fit_pcr_1119_symptomatic_strongprior.rds")
...
sym_1119_testing_maineffect <- stan(file = "MRP_pcr_prediction_pps_symptomatic_main_1119.stan", data = patient_pps_data_symptomatic_main_effect,refresh=0, cores=7,iter = 10000,chains=2)
saveRDS(sym_1119_testing_maineffect, "fit_pcr_1119_symptomatic_maineffect.rds")
...
fit_pcr_symptomatic=readRDS("fit_pcr_1119_symptomatic.rds")
fit_pcr_symptomatic = sym_1119_testing
fit_pcr_symptomatic = sym_1119_testing_1
fit_pcr_symptomatic = sym_1119_testing_strongprior
fit_pcr_symptomatic = sym_1119_testing_maineffect
...
MRP_symptomatic_plot <- ggplot(data=est_raw_symptomatic, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(est_raw_symptomatic$week),to=max(est_raw_symptomatic$week), by = 1),labels = c("May5 - May11",
                                                                ...
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
    theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
ggsave(MRP_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_mrp_sym_est.png")

MRP_sd_symptomatic_plot <- ggplot(data=est_raw_symptomatic, aes(x=week,y=est,group=type,colour=type))+
  geom_point(aes(shape=type))+
  geom_line(aes(linetype=type,colour=type),alpha=0.4)+
  geom_errorbar(aes(ymin=est-sd, ymax=est+sd,linetype=type), width=0.1,alpha=0.4)+
  labs (title = "", x = "Week", y = "PCR Prevalence")+
...
ggsave(MRP_sd_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_mrp_sym_sd.png")
...
#     theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
# 
# ggsave(MRP_CI_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/pcr_mrp_sym_ci.png")
...
#saveRDS(fit_IGG, "fit_igg1021.rds")

fit_IGG = readRDS("fit_igg1027.rds")
#
...
IGG_est_raw = data.frame(cbind(
  rep(c("Raw","MRP-hospital","MRP-community"),each=length(unique(IGG_prevalence_by_week$igg_week_ind))),
  rep(IGG_prevalence_by_week$igg_week_ind,3),
  c(IGG_prevalence_by_week$prevalence,
    apply(rstan::extract(fit_IGG,"p_avg_h", permuted = TRUE)[[1]],2,mean),
    apply(rstan::extract(fit_IGG,"p_avg_c", permuted = TRUE)[[1]],2,mean)),
  c(rep(0,length(unique(IGG_prevalence_by_week$igg_week_ind))),apply(rstan::extract(fit_IGG,"p_avg_h", permuted = TRUE)[[1]],2,sd),
    apply(rstan::extract(fit_IGG,"p_avg_c", permuted = TRUE)[[1]],2,sd))
  ))

names(IGG_est_raw)=c("type","week","est","sd")
IGG_est_raw$week = as.numeric(as.character(IGG_est_raw$week))
IGG_est_raw$est = as.numeric(as.character(IGG_est_raw$est))
IGG_est_raw$sd = as.numeric(as.character(IGG_est_raw$sd))
...
MRP_IGG_plot <- ggplot(data=IGG_est_raw, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(IGG_est_raw$week),to=max(IGG_est_raw$week), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                ...
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
ggsave(MRP_IGG_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_est.png")

MRP_IGG_sd_plot <- ggplot(data=IGG_est_raw, aes(x=week,y=est,group=type,colour=type))+
  geom_point(aes(shape=type))+
  geom_line(aes(linetype=type,colour=type),alpha=0.4)+
  geom_errorbar(aes(ymin=est-sd, ymax=est+sd,linetype=type), width=0.1,alpha=0.4)+
  labs (title = "", x = "Week", y = "IGG Prevalence")+
...
ggsave(MRP_IGG_sd_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_sd.png")
...
#   theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
# 
# ggsave(MRP_IGG_CI_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_ci.png")
grid.arrange(MRP_IGG_plot,
             MRP_IGG_sd_plot, nrow = 1)
...
#saveRDS(fit_IGG_symptomatic, "fit_igg_symptomatic1021.rds")

fit_IGG_symptomatic = readRDS("fit_igg_symptomatic1027.rds")
...
IGG_est_raw_symptomatic = data.frame(cbind(rep(c("Raw","MRP-hospital","MRP-community"),each=length(unique(IGG_prevalence_by_week_symptomatic$igg_week_ind))),rep(IGG_prevalence_by_week_symptomatic$igg_week_ind,3),c(IGG_prevalence_by_week_symptomatic$prevalence,apply(rstan::extract(fit_IGG_symptomatic,"p_avg_h", permuted = TRUE)[[1]],2,mean),apply(rstan::extract(fit_IGG_symptomatic,"p_avg_c", permuted = TRUE)[[1]],2,mean)),                                      c(rep(0,length(unique(IGG_prevalence_by_week_symptomatic$igg_week_ind))),apply(rstan::extract(fit_IGG_symptomatic,"p_avg_h", permuted = TRUE)[[1]],2,sd),apply(rstan::extract(fit_IGG_symptomatic,"p_avg_c", permuted = TRUE)[[1]],2,sd))))
names(IGG_est_raw_symptomatic)=c("type","week","est","sd")
IGG_est_raw_symptomatic$week = as.numeric(as.character(IGG_est_raw_symptomatic$week))
IGG_est_raw_symptomatic$est = as.numeric(as.character(IGG_est_raw_symptomatic$est))
IGG_est_raw_symptomatic$sd = as.numeric(as.character(IGG_est_raw_symptomatic$sd))
...
MRP_IGG_symptomatic_plot <- ggplot(data=IGG_est_raw_symptomatic, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(IGG_est_raw_symptomatic$week),to=max(IGG_est_raw_symptomatic$week), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                ...
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12", "Oct13 - Oct19", "Oct20 - Oct26"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
ggsave(MRP_IGG_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_sym_est.png")

MRP_IGG_sd_symptomatic_plot <- ggplot(data=IGG_est_raw_symptomatic, aes(x=week,y=est,group=type,colour=type))+
  geom_point(aes(shape=type))+
  geom_line(aes(linetype=type,colour=type),alpha=0.4)+
  geom_errorbar(aes(ymin=est-sd, ymax=est+sd,linetype=type), width=0.1,alpha=0.4)+
  labs (title = "", x = "Week", y = "IGG Prevalence")+
...
ggsave(MRP_IGG_sd_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_sym_sd.png")
...
#   theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
# 
# ggsave(MRP_IGG_CI_symptomatic_plot, width = 5, height = 3, units = "in", device='png',file="plot/igg_mrp_sym_ci.png")
# 
...
week.inc = count_comparison$PCR_prevalence_by_week.pcr_week_ind.2.30.
combine = data.frame(cbind(week.inc,                           count_comparison$ratio_approximated,
...
combine_v_sym = data.frame(cbind(
  rep(combine$week,3),
  c(rep("state",length(combine$week)),rep("hosp-sym",length(combine$week)),rep("com-sym",length(combine$week))),
  c(combine$state,combine$Hospsym,combine$Comsym)
))
...
names(combine_v_sym) = c("week","type","est")
combine_v_sym$week = as.numeric(as.character(combine_v_sym$week))
combine_v_sym$est = as.numeric(as.character(combine_v_sym$est))
...
combine_plot = ggplot(data=combine_v, aes(x=week,y=est,group=type,colour=type))+
  ...
  labs (title = "", x = "Week", y = "PCR Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.5))+
  scale_x_continuous(breaks=seq(from=min(combine_v$week),to=max(combine_v$week), by = 1),labels = c("May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
combine_plot_sym = ggplot(data=combine_v_sym, aes(x=week,y=est,group=type,colour=type))+
  geom_point(aes(shape=type))+
  geom_line(aes(linetype=type),alpha=0.4)+
  labs (title = "", x = "Week", y = "PCR Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.5))+
...
ggsave(combine_plot_sym, width = 5, height = 3, units = "in", device='png',file="plot/pcr_compare_sym.png")             
```
...
ggplot(data =state_data_by_day, aes(x = day_ind,y = average_prevalence))+
  geom_point() +
  geom_line()+
  labs(title = "COVID-19 prevalence in District 1, Indiana", x = "Days", y = "7-Day Averag Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.4))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
...
ggplot(data =PCR_prevalence_by_day, aes(x = pcr_day_ind,y = average_prevalence_asym))+
  geom_point() +
  geom_line()+
  labs(title = "Asymptomatic Patients", x = "Days", y = "7-Day Averag Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.05))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
...
ggplot(data =PCR_prevalence_by_day_symptomatic, aes(x = pcr_day_ind,y = average_prevalence_sym))+
  geom_point() +
  geom_line()+
  labs(title = "Symptomatic Patients", x = "Days", y = "7-Day Averag Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.4))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
...
sex_ratio <- data.frame(rep(seq(from = 20, to = 47, by = 1), 2),
                   c(sex_prevalence$prevalence_male[3:30]/sex_prevalence_symptotic$prevalence_male[2:29], sex_prevalence$prevalence_female[3:30]/sex_prevalence_symptotic$prevalence_female[2:29]),
                   c(rep("Male", length(sex_prevalence_symptotic$pcr_week_ind[2:29])),rep("Female", length(sex_prevalence_symptotic$pcr_week_ind[2:29]))))
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = "PCR Asym/Sym")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=20,to=47, by = 1),labels = c(
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
#Starting from Week 20 to avoid 0/0
race_ratio <- data.frame(rep(seq(from = 20, to = 47, by = 1), 3),
                         c(race_prevalence$prevalence_white[3:30]/race_prevalence_symptotic$prevalence_white[2:29],
                           race_prevalence$prevalence_black[3:30]/race_prevalence_symptotic$prevalence_black[2:29],
                           race_prevalence$prevalence_other[3:30]/race_prevalence_symptotic$prevalence_other[2:29]),
                         c(rep("White", length(race_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Black", length(race_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Other", length(race_prevalence_symptotic$pcr_week_ind[2:29]))))
...
#Starting from Week 20 to avoid 0/0
age_ratio <- data.frame(rep(seq(from = 20, to = 47, by = 1), 5),
                         c(age_prevalence$prevalence_age1[3:30]/age_prevalence_symptotic$prevalence_age1[2:29],
                           age_prevalence$prevalence_age2[3:30]/age_prevalence_symptotic$prevalence_age2[2:29],
                           age_prevalence$prevalence_age3[3:30]/age_prevalence_symptotic$prevalence_age3[2:29],
                           age_prevalence$prevalence_age4[3:30]/age_prevalence_symptotic$prevalence_age4[2:29],
                           age_prevalence$prevalence_age5[3:30]/age_prevalence_symptotic$prevalence_age5[2:29]),
                         c(rep("Age0_17", length(age_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Age18_39", length(age_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Age40_64", length(age_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Age65_75", length(age_prevalence_symptotic$pcr_week_ind[2:29])),
                           rep("Age75+", length(age_prevalence_symptotic$pcr_week_ind[2:29]))))
...
spline_pred=data.frame(cbind(rep(c("MRP-hospital","MRP-community"), each=length(week_pred)),
  rep(week_pred,2),
  c(apply(rstan::extract(mrp_pcr_fit,"p_avg_h_pred", permuted = TRUE)[[1]],2,mean),
  apply(rstan::extract(mrp_pcr_fit,"p_avg_c_pred", permuted = TRUE)[[1]],2,mean)),
  c(apply(rstan::extract(mrp_pcr_fit,"p_avg_h_pred", permuted = TRUE)[[1]],2,quantile,probs=0.25),
  apply(rstan::extract(mrp_pcr_fit,"p_avg_c_pred", permuted = TRUE)[[1]],2,quantile,probs=0.25)),
  c(apply(rstan::extract(mrp_pcr_fit,"p_avg_h_pred", permuted = TRUE)[[1]],2,quantile,probs=0.75),
  apply(rstan::extract(mrp_pcr_fit,"p_avg_c_pred", permuted = TRUE)[[1]],2,quantile,probs=0.75))))
...
ggplot(data=spline_pred, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(spline_pred$week),to=max(spline_pred$week), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 - Nov9","Nov10 - Nov16","Nov17 - Nov23", "Nov24 - Nov30", "Dec1 - Dec7"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
ggplot(data=est_raw_1, aes(x=week,y=est,group=type,colour=type))+
  ...
  scale_x_continuous(breaks=seq(from=min(est_raw_1$week),to=max(est_raw_1$week), by = 1),labels = c("Apr28 - May4","May5 - May11",
                                                                "May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 - Nov9"))+
  theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),legend.position="bottom",legend.title=element_blank(),legend.text = element_text(size=9))
...
fit_pcr_spline <- readRDS("fit_pcr_spline_1119_asymptomatic.rds")
...
ggplot()+
  ...
  labs (title = "Posterior predictive prevalence vs Raw prevalence for asymptomatic patient", x = "Week", y = " Prevalence")+
...
##########Posterior prediction distribution
fit_pcr_spline_symptomatic  <- readRDS("fit_pcr_spline_1119_symptomatic.rds")
...
ggplot()+
  ...
  labs (title = "Posterior predictive prevalence vs Raw prevalence for symptomatic patient", x = "Week", y = " Prevalence")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.6))+
...
```{r}
mrp_pcr_fit <- readRDS('mrp_pcr_fit.rds')
fit_pcr_symptomatic  <- readRDS('fit_pcr_symptomatic.rds')
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Gender in Hosp")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_hosp_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9","Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Gender in State")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_state_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Race in Hosp")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_hosp_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9","Nov10 - Nov16", "Nov17 - Nov23"))+
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Race in State")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_state_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9","Nov10 - Nov16", "Nov17 - Nov23"))+
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Age in Hosp")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_hosp_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
...
ggplot()+
  ...
  labs (title = "", x = "Week", y = " MRP Ratio of Asym/Sym in Age in State")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.25))+
  scale_x_continuous(breaks=seq(from=1,to=length(male_mrp_state_sym), by = 1),labels = c(
                                                                "May5 - May11","May12 - May18","May19 - May25","May26 - Jun1","Jun2 - Jun8","Jun9 - Jun15","Jun16 - Jun22","Jun23 - Jun29","Jun30 - July6","July7 - July13","July14 - July20",
                                                                "July21 - July27","July28 - Aug3","Aug4 - Aug10","Aug11 - Aug17","Aug18 - Aug24","Aug25 - Aug31","Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2","Nov3 -Nov9", "Nov10 - Nov16"))+
  theme_bw()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))
...
names(list_age_stan) <-  c("age_w1","age_w2","age_w3","age_w4","age_w5","age_w6","age_w7","age_w8","age_w9","age_w10","age_w11","age_w12","age_w13","age_w14","age_w15","age_w16","age_w17","age_w18","age_w19","age_w20","age_w21","age_w22","age_w23","age_w24","age_w25","age_w26","age_w27","age_w28","age_w29","age_w30")
names(list_race_stan) <-  c("race_w1","race_w2","race_w3","race_w4","race_w5","race_w6","race_w7","race_w8","race_w9","race_w10","race_w11","race_w12","race_w13","race_w14","race_w15","race_w16","race_w17","race_w18","race_w19","race_w20","race_w21","race_w22","race_w23","race_w24","race_w25","race_w26","race_w27","race_w28","race_w29","race_w30")
names(list_sex_stan) <-  c("male_w1","male_w2","male_w3","male_w4","male_w5","male_w6","male_w7","male_w8","male_w9","male_w10","male_w11","male_w12","male_w13","male_w14","male_w15","male_w16","male_w17","male_w18","male_w19","male_w20","male_w21","male_w22","male_w23","male_w24","male_w25","male_w26","male_w27","male_w28","male_w29","male_w30")
names(list_week) <-  c("time_w1","time_w2","time_w3","time_w4","time_w5","time_w6","time_w7","time_w8","time_w9","time_w10","time_w11","time_w12","time_w13","time_w14","time_w15","time_w16","time_w17","time_w18","time_w19","time_w20","time_w21","time_w22","time_w23","time_w24","time_w25","time_w26","time_w27","time_w28","time_w29","time_w30")
supplementary_info <- list(offset = 0.5, n1 = N[1],n2 = N[2], n3 = N[3], n4 = N[4], n5 = N[5], n6 = N[6], n7 = N[7], n8 = N[8], n9 = N[9], n10 = N[10], n11 = N[11], n12 = N[12], n13 = N[13], n14 = N[14], n15 = N[15], n16 = N[16], n17 = N[17], n18 = N[18], n19 = N[19], n20 = N[20], n21 = N[21], n22 = N[22], n23 = N[23], n24 = N[24], n25 = N[25], n26 = N[26], n27 = N[27], n28 = N[28], n29 = N[29], n30 = N[30])

patient_pps_data <- pcr_data_stan_Andrew %>% append(list_sex_age) %>% append(list_county_stan) %>% append(list_age_stan) %>% append(list_race_stan) %>% append(list_sex_stan) %>% append(list_week) %>% append(supplementary_info)
#fit_pcr_1107 <- stan(file = "MRP_pcr_prediction_pps.stan", data = patient_pps_data,refresh=0, cores=7,iter = 10000,chains=2)
#saveRDS(fit_pcr_1107, "fit_pcr_1107.rds")
mrp_pcr_fit <- readRDS("fit_pcr_1119_asymptomatic.rds")
pps_raw <- as.data.frame(mrp_pcr_fit, pars = c("mean_y_rep_w1","mean_y_rep_w2","mean_y_rep_w3","mean_y_rep_w4","mean_y_rep_w5","mean_y_rep_w6","mean_y_rep_w7","mean_y_rep_w8","mean_y_rep_w9","mean_y_rep_w10","mean_y_rep_w11","mean_y_rep_w12","mean_y_rep_w13","mean_y_rep_w14","mean_y_rep_w15","mean_y_rep_w16","mean_y_rep_w17","mean_y_rep_w18","mean_y_rep_w19","mean_y_rep_w20","mean_y_rep_w21","mean_y_rep_w22","mean_y_rep_w23","mean_y_rep_w24","mean_y_rep_w25","mean_y_rep_w26","mean_y_rep_w27","mean_y_rep_w28","mean_y_rep_w29","mean_y_rep_w30"))
...
names(list_sex_age_symptomatic) <-  c("sex_age_w1","sex_age_w2","sex_age_w3","sex_age_w4","sex_age_w5","sex_age_w6","sex_age_w7","sex_age_w8","sex_age_w9","sex_age_w10","sex_age_w11","sex_age_w12","sex_age_w13","sex_age_w14","sex_age_w15","sex_age_w16","sex_age_w17","sex_age_w18","sex_age_w19","sex_age_w20","sex_age_w21","sex_age_w22","sex_age_w23","sex_age_w24","sex_age_w25","sex_age_w26","sex_age_w27","sex_age_w28","sex_age_w29")
names(list_county_stan_symptomatic) <-  c("county_w1","county_w2","county_w3","county_w4","county_w5","county_w6","county_w7","county_w8","county_w9","county_w10","county_w11","county_w12","county_w13","county_w14","county_w15","county_w16","county_w17","county_w18","county_w19","county_w20","county_w21","county_w22","county_w23","county_w24","county_w25","county_w26","county_w27","county_w28","county_w29")
names(list_age_stan_symptomatic) <-  c("age_w1","age_w2","age_w3","age_w4","age_w5","age_w6","age_w7","age_w8","age_w9","age_w10","age_w11","age_w12","age_w13","age_w14","age_w15","age_w16","age_w17","age_w18","age_w19","age_w20","age_w21","age_w22","age_w23","age_w24","age_w25","age_w26","age_w27","age_w28","age_w29")
names(list_race_stan_symptomatic) <-  c("race_w1","race_w2","race_w3","race_w4","race_w5","race_w6","race_w7","race_w8","race_w9","race_w10","race_w11","race_w12","race_w13","race_w14","race_w15","race_w16","race_w17","race_w18","race_w19","race_w20","race_w21","race_w22","race_w23","race_w24","race_w25","race_w26","race_w27","race_w28","race_w29")
names(list_sex_stan_symptomatic) <-  c("male_w1","male_w2","male_w3","male_w4","male_w5","male_w6","male_w7","male_w8","male_w9","male_w10","male_w11","male_w12","male_w13","male_w14","male_w15","male_w16","male_w17","male_w18","male_w19","male_w20","male_w21","male_w22","male_w23","male_w24","male_w25","male_w26","male_w27","male_w28","male_w29")
names(list_week_symptomatic) <-  c("time_w1","time_w2","time_w3","time_w4","time_w5","time_w6","time_w7","time_w8","time_w9","time_w10","time_w11","time_w12","time_w13","time_w14","time_w15","time_w16","time_w17","time_w18","time_w19","time_w20","time_w21","time_w22","time_w23","time_w24","time_w25","time_w26","time_w27","time_w28","time_w29")
supplementary_info_symptomatic <- list(offset = 0.5, n1 = N_symptomatic[1],n2 = N_symptomatic[2], n3 = N_symptomatic[3], n4 = N_symptomatic[4], n5 = N_symptomatic[5], n6 = N_symptomatic[6], n7 = N_symptomatic[7], n8 = N_symptomatic[8], n9 = N_symptomatic[9], n10 = N_symptomatic[10], n11 = N_symptomatic[11], n12 = N_symptomatic[12], n13 = N_symptomatic[13], n14 = N_symptomatic[14], n15 = N_symptomatic[15], n16 = N_symptomatic[16], n17 = N_symptomatic[17], n18 = N_symptomatic[18], n19 = N_symptomatic[19], n20 = N_symptomatic[20], n21 = N_symptomatic[21], n22 = N_symptomatic[22], n23 = N_symptomatic[23], n24 = N_symptomatic[24], n25 = N_symptomatic[25], n26 = N_symptomatic[26], n27 = N_symptomatic[27], n28 = N_symptomatic[28], n29 = N_symptomatic[29])
...
```{r}
state_hospitalization <- read.csv("./hospitalization_1121.csv")
...
state_hospitalization_mrp <- data.frame(rep(seq(from = 1, to = 30, by = 1), 3),
                                        c(state_hospitalization$bed_avg, state_hospitalization$vent_ave, apply(rstan::extract(mrp_pcr_fit,"p_avg_c", permuted = TRUE)[[1]],2,mean)),
                                        c(rep("State_bed_occupancy", 30),rep("State_ventilator_occupancy", 30),rep("MRP_community",30)))
...
ggplot()+
  ...
  labs(title = "Comparasion of State hospitalization with MRP for asymptomatic patient", x = "Week", y = "Percentage")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.5))+
  scale_x_continuous(breaks=seq(from=1,to=30, by = 1),labels = c(
...
```{r}
district1_hospitalization<- read.csv("./Distrcit1_hospitalization_1121.csv")
...
### County Level test
```{r}
county_data <- read.csv("./county_1121.csv")
...
fit_pcr_spline_1119_asymptomatic <- stan(file = "spline_pps_1119.stan", data =patient_pps_data_spline ,refresh=0, cores=7,iter = 10000,chains=2) 
saveRDS(fit_pcr_spline_1119_asymptomatic, "fit_pcr_spline_1119_asymptomatic.rds")
fit_pcr_spline_1119_symptomatic <- stan(file = "spline_pps_symptomatic_1119.stan", data =patient_pps_data_spline_symptomatic ,refresh=0, cores=7,iter = 10000,chains=2) 
saveRDS(fit_pcr_spline_1119_symptomatic, "fit_pcr_spline_1119_symptomatic.rds")

fit_pcr_1119_asymptomatic <- stan(file = "MRP_pcr_prediction_pps_1119.stan", data = patient_pps_data,refresh=0, cores=7,iter = 10000,chains=2)
saveRDS(fit_pcr_1119_asymptomatic, "fit_pcr_1119_asymptomatic.rds")
fit_pcr_1119_symptomatic <- stan(file = "MRP_pcr_prediction_pps_symptomatic_1119.stan", data = patient_pps_data_symptomatic,refresh=0, cores=7,iter = 10000,chains=2)
saveRDS(fit_pcr_1119_symptomatic, "fit_pcr_1119_symptomatic.rds")
```
...
#a
MRP_Positivity <- data.frame(rep(seq(from = 36, to = 47, by = 1), 2),
                                        c(MRP_summary$est, County_summary$prevalence),
                                        c(rep("MRP", 11),rep("Lake&Porter Positivity",11)))
...
plot_a<- ggplot()+
  ...
  labs(title = "MRP vs. Lake&Porter Positivity", x = "Week", y = "Percentage")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.4))+
  scale_x_continuous(breaks=seq(from=36,to=47, by = 1),labels = c(
"Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))
#b
plot_b <- 
  ...
  scale_x_continuous(breaks=seq(from=36,to=47, by = 1),labels = c("Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  scale_y_continuous("Lake&Porter #Positivity Patient", sec.axis = sec_axis(~./100000, name = "MRP"))+
  labs(title = "MRP vs. Lake&Porter #Positive Patient", x = "Week")+
...
#c
plot_c <- ggplot()+
  geom_line(data =MRP_summary, aes(x = week, y = est*2000, color = "MRP"))+
  ...
  scale_x_continuous(breaks=seq(from=36,to=47, by = 1),labels = c("Sep1 - Sep7","Sep8 - Sep14","Sep15 - Sep21", "Sep22 - Sep28", "Sep29 - Oct5", "Oct6 - Oct12","Oct13 - Oct19", "Oct20 - Oct26", "Oct27 - Nov2", "Nov3 - Nov9", "Nov10 - Nov16", "Nov17 - Nov23"))+
  scale_y_continuous("Lake&Porter Hospitalization Count", sec.axis = sec_axis(~./2000, name = "MRP"))+
  labs(title = "MRP vs. Lake&Porter Hospitalization Count", x = "Week")+
  scale_colour_manual(values = c("blue","red"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))+
...
plot_d <- ggplot()+
  ...
  labs(title = "Lake&Porter positivity vs. Lake&Porter Hospitalization Count", x = "Week")+
  scale_colour_manual(values = c("blue","red"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))+
...
plot_e <- ggplot()+
  ...
  labs(title = "Lake&Porter #Positive Patient vs. Lake&Porter Hospitalization Count", x = "Week")+
  scale_colour_manual(values = c("blue","red"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),axis.text.y = element_text(size=9))+
  theme(legend.position="bottom",legend.text = element_text(size=8))+
...
County_summary_ma <- county_data_by_day %>% 
  filter(day_ind>= 245 & day_ind<=320)
...
Rawdata_summary_ma <- PCR_prevalence_by_day %>% 
  filter(pcr_day_ind>= 245 & pcr_day_ind<=320) 
...
original_model <- glm(y_response ~ sex_stan + age_stan_relevel_symptomatic + as.factor(race_stan) + factor(sex_stan):age_stan_relevel_symptomatic + factor(county), family = "binomial", data = patient_data_symptomatic) 
