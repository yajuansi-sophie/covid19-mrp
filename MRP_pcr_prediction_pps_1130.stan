...
//posterior predictive check
  ...
  vector[n29] logit_p_w29 = b[1] + b[2]*male_w29  + a_race[race_w29] + a_age[age_w29] + (male_w29 + offset) .* a_sexage[sex_age_w29] + a_time[time_w29] + a_county[county_w29];
  int<lower = 0> y_rep_w29[n29] = bernoulli_logit_rng(logit_p_w29*sens[1] + (1-logit_p_w29)*(1-spec[1]));
  real<lower = 0> mean_y_rep_w29 = mean(to_vector(y_rep_w29));
  real<lower = 0> sd_y_rep_w29 = sd(to_vector(y_rep_w29));
  vector[n30] logit_p_w30 = b[1] + b[2]*male_w30  + a_race[race_w30] + a_age[age_w30] + (male_w30 + offset) .* a_sexage[sex_age_w30] + a_time[time_w30] + a_county[county_w30];
  int<lower = 0> y_rep_w30[n30] = bernoulli_logit_rng(logit_p_w30*sens[1] + (1-logit_p_w30)*(1-spec[1]));
  real<lower = 0> mean_y_rep_w30 = mean(to_vector(y_rep_w30));
  real<lower = 0> sd_y_rep_w30 = sd(to_vector(y_rep_w30));
  vector[n31] logit_p_w31 = b[1] + b[2]*male_w31  + a_race[race_w31] + a_age[age_w31] + (male_w31 + offset) .* a_sexage[sex_age_w31] + a_time[time_w31] + a_county[county_w31];
  int<lower = 0> y_rep_w31[n31] = bernoulli_logit_rng(logit_p_w31*sens[1] + (1-logit_p_w31)*(1-spec[1]));
